...
客户端始终校验证书。Python 系统 CA 库为空且未显式设置 `SSL_CERT_FILE` / `SSL_CERT_DIR` 时，自动加载项目依赖 `certifi` 的 CA 包；已配置的系统或自定义证书库保持优先。运行提交器的进程还必须具备网络访问权限。浏览器或 `curl` 成功不代表 Python 使用了相同的证书库或网络权限。
...
网络错误响应包含 `error_kind`（`tls_certificate`、`dns`、`timeout`、`connection` 或 `network`）、请求方法与路径、耗时、异常类型及可用的 `errno`，不回显底层异常消息中的凭据或代理地址。HTTP 错误仍保留 `http_status`。证书或 DNS 错误应先修复本地运行环境；只有结合服务器日志确认写请求到达并超时或返回限流状态，才能判断为服务器写接口问题。
