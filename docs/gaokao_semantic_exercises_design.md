...
- 独立 AI 审计逐项判断真实义项、语法词形和语境可接受性，并检查题干自然度、唯一线索和答案泄露；只从语法平行且语义错误的候选中选取 3 个。
- 只有同时记录当前 `generation_prompt_version`、`audit_version` 和独立 `quality_gate` 的题目才可发布。候选不足时批量重生成一次，仍失败则进入后续自动补全队列。
- 词汇导入只暂存候选；待审与失败合计达到 30 后，由后台在 DeepSeek 低峰时段按每批最多 10 个异步审计或重生成。
