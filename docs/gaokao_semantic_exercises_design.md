...
- 四个选项均只展示一个核心义项，不附词性缩写；最长与最短选项最多相差 2 个汉字，避免通过字数或标点猜答案。
...
- 生成 Prompt 每题输出 6 个中文和 6 个英文候选，并要求模型在输出最终 JSON 前自行检查；程序保留完整候选池，不因结构合格而直接发布。
- 英文识义的干扰项不得命中任一真实义项、同义表达、上位概念或包含正确义项的表述。
- 英文识义的正确项与干扰项必须保持单义、相近字数和相同信息粒度，多义完整释义保留在学习卡与词库中。
- 程序对 JSON 结构、候选数量和重复项、语境长度、答案精确词形及中文字段执行确定性校验；失败的题不进入审计。
- 审计 v5 将识义与语境分开盲审，选项打乱且不标注答案；语境请求不携带目标词、释义或讲解。布尔数组与实际选项顺序对齐，程序只从语法平行且语义错误的候选中选取 3 个；最终四选项另行校对译文、解析和答案与目标词的对应关系。
- 审计按题保留有效响应，只对格式异常、遗漏或截断的题缩小批次重试一次；识义明确拒绝时停止该题后续审计。配置和网络故障停止任务，不拆批，也不消耗逐题质量预算。
- 只有从未生成且无历史处理痕迹的词允许首次整题生成。已有题目、候选、拒绝记录、原始草稿或失败痕迹均阻止再次整题生成，`--force` 也不例外；兼容旧题重审，候选续审。语义拒绝、结构不兼容或来源语义变化时保留原内容，转人工处理。
- 缺失或错误的中文译文、解析只使用指定字段的短请求修复，不重新输出题干和候选。审计断点绑定题面、讲解和模型配置；讲解修复仅使对应校对失效，其余有效阶段可复用。
- 只有同时记录当前 `generation_prompt_version`、`audit_version` 和独立 `quality_gate`，且匹配当前语义来源的题目才可发布。供题、发布和后台审计核对来源；兼容指纹不包含音标、级别和例句中文译文，缺少指纹的旧题可通过重审补齐。
- 手工刷新默认优先未尝试项并跳过历史失败，`--retry-failed` 才将失败项纳入续审或字段修复。按词分块读写使用 SQLite 的 `word_key` 索引；旧库只补索引，不重写题目、断点或版本号。
- 词汇导入只暂存候选；到期任务达到 30 个或最老题等待 1 小时后，后台在低峰时段按每请求最多 10 个处理。每次实际 HTTP 请求及重试都检查时段，跨入高峰即暂停并保留断点。可重试的逐题失败退避，累计 5 次后转人工处理；传输故障和低峰暂停不计入预算，历史未标记失败不会自动领取。
