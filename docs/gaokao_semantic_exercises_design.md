...
- 生成 Prompt 要求模型在输出最终 JSON 前，内部逐项代入识义题和语境题的四个选项；存在多个合理答案时先重写题干或更换干扰项。
- 英文识义的干扰项不得命中任一真实义项、同义表达、上位概念或包含正确义项的表述。
- 英文识义的正确项与干扰项必须保持单义、相近字数和相同信息粒度，多义完整释义保留在学习卡与词库中。
- 程序对 JSON 结构、选项数量和重复项、语境长度、答案精确词形及中文字段执行确定性校验；失败的题不发布。
- 通过确定性校验的题目直接发布，并记录 `generation_prompt_version` 和 `quality_gate`。该策略以降低全量生成调用量为目标，接受少量无法由程序识别的语义歧义。
- 独立 AI 审查不再是发布前置步骤；旧候选审查能力保留用于兼容，正式题库通过人工抽检、用户反馈和版本化下线持续修正。
