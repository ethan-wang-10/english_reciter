...
- 审计按题保留有效响应，只对异常题缩小批次重试一次。重新运行生成任务会续审已有候选；讲解也参与候选指纹，避免旧审计覆盖已修改的讲解。
- 只有同时记录当前 `generation_prompt_version`、`audit_version` 和独立 `quality_gate` 的题目才可发布。候选不足时批量重生成一次，修复请求会携带首轮的确切失败原因；仍失败则进入后续自动补全队列。
- 词汇导入只暂存候选；到期任务达到 30 个或最老题等待 1 小时后，后台在低峰时段按每请求最多 10 个处理。失败逐题退避，累计 5 次后标记待人工处理；历史未标记失败不会自动领取。
