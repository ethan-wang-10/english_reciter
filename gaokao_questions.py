...
def generate_question_records(
    ...
    reply = chat([{"role": "user", "content": prompt}], min(8192, 1200 + len(sources) * 450))
    parsed = _extract_json_array(reply or "")
    if parsed is None:
        return {}, {source["english"]: "AI response is missing a valid JSON array" for source in sources}
    raw_by_key = {
...
def generate_and_persist(
    ...
    *,
    ...
    records, errors = generate_question_records(pending, chat)
